...
def merge_consecutive_u3_gates(qc: QuantumCircuit) -> QuantumCircuit:
    ...
    Uses multiple passes to ensure all mergeable gates are merged.
    ...
    """
    # Use multiple passes to ensure all mergeable gates are merged
    current_qc = qc
    prev_size = current_qc.size()
    max_passes = 10
    pass_num = 0
    ...
    while pass_num < max_passes:
        ...
        instructions = list(current_qc.data)
        
        # Build optimized circuit
        qc_opt = QuantumCircuit(current_qc.num_qubits)
        ...
        while i < len(instructions):
            ...
            if inst.operation.name == 'u':
                # Get qubit index
                qubit = current_qc.qubits.index(inst.qubits[0])
                ...
                while j < len(instructions):
                    ...
                    affected_qubits = [current_qc.qubits.index(q) for q in next_inst.qubits]
        ...
        new_size = qc_opt.size()
        if new_size == prev_size:
            # No more merges possible, stop
            break
        prev_size = new_size
        current_qc = qc_opt
