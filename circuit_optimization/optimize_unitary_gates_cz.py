...
../gates/unitary_gates/tensor{i}.pt, using a U3+CZ-based structure.

This is a pure PyTorch implementation (no Qiskit) to ensure consistency
with PyTorch's tensor encoding.

Results are saved under ../gates/decomposed_gates_cz/gate_index{i}/:
...
def cz_matrix() -> torch.Tensor:
    """
    Build CZ (Controlled-Z) gate matrix using PyTorch's row-major encoding:
    CZ = [
        [1, 0, 0, 0],   # |00⟩ -> |00⟩
        [0, 1, 0, 0],   # |01⟩ -> |01⟩
        [0, 0, 1, 0],   # |10⟩ -> |10⟩
    ...
    matrix[2, 2] = 1.0   # |10⟩ -> |10⟩
    matrix[3, 3] = -1.0  # |11⟩ -> -|11⟩
    return matrix
...
def build_ansatz_matrix(params: np.ndarray,
                       num_qubits: int = 3,
                       num_layers: int = 1,
                       cz_pairs: List[List[int]] = None) -> Tuple[torch.Tensor, List[str]]:
    """
    ...
    Args:
        ...
        cz_pairs: List of CZ gate qubit pairs, e.g., [[0,1], [1,2]] or [[0,2], [1,2]].
                  If None, defaults to [[0,1], [1,2]] for 3 qubits, [[0,1]] for 2 qubits.
                  Each pair is [q0, q1] (CZ is symmetric, order doesn't matter).
    
    Returns:
        (matrix, gate_descriptions): Full unitary matrix and list of gate descriptions
    """
    # Set default CZ pairs if not provided
    if cz_pairs is None:
        if num_qubits >= 3:
            cz_pairs = [[0, 1], [1, 2]]
        elif num_qubits >= 2:
    ...
    for pair in cz_pairs:
        if len(pair) != 2:
            raise ValueError(f"Each CZ pair must have exactly 2 qubits, got {pair}")
        if pair[0] < 0 or pair[0] >= num_qubits or pair[1] < 0 or pair[1] >= num_qubits:
            raise ValueError(f"CZ pair {pair} contains invalid qubit indices for {num_qubits} qubits")
    ...
    if num_layers == 0:
        ...
        for layer in range(num_layers):
            # All layers start with U3(all) before the first CZ
            for q in range(num_qubits):
                tx, ty, tz = params_t[idx:idx + 3]
                u3_gate = u3_matrix(tx, ty, tz)
                matrix = apply_single_qubit_gate_to_matrix(matrix, u3_gate, q, num_qubits)
            ...
            # Apply CZ gates and U3 blocks according to cz_pairs
            for i, (q0, q1) in enumerate(cz_pairs):
                # Apply CZ gate (symmetric, order doesn't matter)
                cz_gate = cz_matrix()
                matrix = apply_two_qubit_gate_to_matrix(matrix, cz_gate, q0, q1, num_qubits)
                gate_descriptions.append(f"op{op_idx}: cz[{q0},{q1}]")
                op_idx += 1
                
                # Apply U3(all) block after each CZ
                for q in range(num_qubits):
                    tx, ty, tz = params_t[idx:idx + 3]
                    u3_gate = u3_matrix(tx, ty, tz)
                    matrix = apply_single_qubit_gate_to_matrix(matrix, u3_gate, q, num_qubits)
...
def count_num_params(num_qubits: int, num_layers: int, cz_pairs: List[List[int]] = None) -> int:
    """
    ...
    Args:
        num_qubits: Number of qubits
        num_layers: Number of layers
        cz_pairs: List of CZ gate qubit pairs. If None, uses default pairs.
    ...
    if num_layers == 0:
        ...
        # Structure:
        # All layers: U3(all) + [CZ + U3(all)] for each CZ pair
        # Each U3(all) block has 3 * num_qubits parameters
        
        params_per_u3_block = 3 * num_qubits  # U3(all) block
        params_per_cz_block = 3 * num_qubits  # U3(all) after each CZ
        num_cz_blocks = len(cz_pairs)
        
        # Each layer: initial U3(all) + CZ blocks
        params_per_layer = params_per_u3_block + num_cz_blocks * params_per_cz_block
...
def objective_function(params: np.ndarray,
                       ...
                       num_layers: int,
                       ci: List[int],
                       cz_pairs: List[List[int]] = None) -> float:
    """Objective function for optimization."""
    matrix, _ = build_ansatz_matrix(params, num_qubits=num_qubits, num_layers=num_layers, cz_pairs=cz_pairs)
...
def optimize_gate(gate_idx: int,
                  max_iter: int = 500,
                  seed: int = 42,
                  cz_pairs: List[List[int]] = None) -> Dict[str, Any]:
    """
    Optimize ansatz to fit unitary_gates/tensor{gate_idx}.pt
    and save result under ../gates/decomposed_gates_cz/gate_index{gate_idx}.
    ...
    # 3. Count parameters
    num_params = count_num_params(num_qubits, num_layers, cz_pairs=cz_pairs)
    print(f"\n3. Ansatz structure:")
    print(f"   Number of layers: {num_layers}")
    print(f"   CZ pairs: {cz_pairs}")
    ...
    # 5. Final evaluation
    print("\n5. Final evaluation...")
    final_matrix, gate_descriptions = build_ansatz_matrix(result.x, num_qubits=num_qubits, num_layers=num_layers, cz_pairs=cz_pairs)
    ...
    # 7. Save results
    print("\n7. Saving results...")
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "gates_2patterns", "decomposed_gates_cz"))
...
def cz_per_gate(num_layers, num_qubits):
    if num_qubits == 1:
        return 0
    elif num_qubits == 2:
        return num_layers
...
def optimize_all_gates(max_iter: int = 500,
                       seed: int = 42,
                       cz_pairs: List[List[int]] = None) -> Dict[int, Dict[str, Any]]:
    """
    ...
    for gi in gate_indices:
        ...
        print(f"{'=' * 70}")
        try:
            res = optimize_gate(gi, max_iter=max_iter, seed=seed, cz_pairs=cz_pairs[gi] if cz_pairs else None)
            all_results[gi] = res
...
if __name__ == "__main__":
    # cz_pairs configuration for each tensor
    # Note: qubit indices must be valid for each tensor's num_qubits
    # tensor0: 1 qubit -> no CZ pairs (empty list)
    ...
    # tensor5: 3 qubits -> can use [0,1],[1,2]
    ...
    optimize_all_gates(max_iter=500, seed=42, cz_pairs=cz_pairs)
    ...
    for idx in sorted(contributed_index.keys()):
        ...
        c = cz_per_gate(num_layers, num_qubits)
        total += c
        print(f"tensor{idx}: qubits={num_qubits}, layers={num_layers}, CZs={c}")

    print("TOTAL CZs across all tensors:", total)
